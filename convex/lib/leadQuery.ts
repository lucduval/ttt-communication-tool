...
export interface LeadFilter {
    ...
    industryId?: string;
    /**
     * Channel the campaign sends on ("email" | "personalised" | "whatsapp").
     * Restricts to leads reachable on that channel (an email for email/
     * personalised, a phone for whatsapp), so the count, list, and Select All
     * stay in sync with the send path that drops unreachable leads.
     */
    channel?: string;
}
...
export function buildLeadFilter(filter: LeadFilter): string {
    ...
    // Channel contactability: only leads reachable on the chosen channel. Keeps
    // the count / list / Select All consistent with the send path, which drops
    // leads without an email (email/personalised) or phone (whatsapp).
    if (filter.channel === "email" || filter.channel === "personalised") {
        expression += ` and ttt_email ne null`;
    } else if (filter.channel === "whatsapp") {
        expression += ` and ttt_mobilephone ne null`;
    }
