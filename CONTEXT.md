...
**Uploaded audience**:
A campaign target resolved from a user-supplied CSV/XLSX file of Dynamics contact ids. The file is parsed client-side — the contact-id column found by layered detection (explicit `contactid` header → Dynamics `(Do Not Modify) <Entity>` export header → GUID-shape auto-detect → user picks the column) — and only the deduped id set is sent to the backend. That set becomes an ordinary **filtered** Recipient Selection carrying `{ contactIds }`, so it re-queries through Contact Query's `contactIds` dimension and is no different downstream from any other filtered campaign: variables, names, and tax figures all come from CRM, and owner scoping is enforced server-side (a user cannot reach contacts they aren't allowed to see by uploading their ids). The file is *only* an id source — it never carries per-recipient template values. The id count shown at upload is the parsed count, not a reachable count: contacts outside the user's access, unreachable on the channel, or no longer in CRM are skipped at send, where the real recipient total self-corrects.
_Avoid_: import, recipient upload, contact list file, mailing list.
...
**Campaign Tally**:
The deep module (`convex/lib/campaignTally.ts`) that owns the single rule turning a campaign's per-recipient message statuses into the canonical counts `{ sent, delivered, failed, pending }`. The `messages` table is the source of truth — each recipient's status is written idempotently — and the tally is a pure function over those statuses, so the tally is the test surface. Both readers cross this one seam and therefore cannot diverge: the campaign-**detail** view recounts live, and the campaign-**list** projection (the `sentCount`/`deliveredCount`/`failedCount` fields on the campaign document) is **set** from the same tally at every batch settle and bounce — *never incremented* — so replays and stuck-batch recovery cannot drift it. `sent` counts the `sent` and `delivered` statuses together (successfully handed to the provider); `delivered` counts only confirmed delivery. The earlier additive counters, mutated across four sites (`markBatchComplete`, `markBatchFailed`, `recordBounces`, and a recovery sweep that decremented nothing), are exactly what this module replaces.
_Avoid_: stats counter, sentCount logic, delivery metrics, deliveredCount drift.
